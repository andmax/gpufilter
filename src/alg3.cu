...
#include "gpudefs.h"
#include "alg0_cpu.h"
#include "alg3_gpu.cuh"

//== IMPLEMENTATION ============================================================

/**
 *  @ingroup api_gpu
 *  @brief Compute Algorithm 3 (order agnostic) with any boundary condition
 *
 *  This function computes R-order recursive filtering with given
 *  weights of an input 2D image using algorithm \f$3_r\f$ and any
 *  given boundary condition.  All choices of boundary conditions
 *  imply border input padding, that is the algorithm 3 is restricted
...
void alg3( float *h_img,
    ...
    if (border == 0) {
        gpufilter::alg3_gpu<false, R>(h_img, width, height, runtimes, w);
    } else if (border > 0) {
        gpufilter::alg3_gpu<true, R>(h_img, width, height, runtimes, w,
                                     border, btype);
    }
