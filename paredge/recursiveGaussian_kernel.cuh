...
__host__ __device__ float4 rgbaIntToFloat(uint c)
{
    float4 rgba;
...
extern "C"
__host__ float rgbaIntToFloat1(uint c)
{
    float4 rgba = rgbaIntToFloat(c);
    return (rgba.x+rgba.y+rgba.z+rgba.w)/4.f;
}

extern "C"
__host__ void convert_rgbaIntToFloat(uint *id, float *od0, float *od1, float *od2, float *od3, int w, int h)
{
    float4 rgba;
    for (int i = 0; i < w*h; ++i) {
        rgba = rgbaIntToFloat(id[i]);
        od0[i] = rgba.x;
        od1[i] = rgba.y;
        od2[i] = rgba.z;
        od3[i] = rgba.w;
...
__host__ float saturatef(const float& v) {
    if (v > 1.f) return 1.f;
    else if (v < 0.f) return 0.f;
    return v;
}

extern "C"
__host__ void convert_rgbaFloatToUchar(uchar *od, float *id0, float *id1, float *id2, float *id3, int w, int h)
{
    for (int i = 0; i < w*h; ++i) {
        od[i*4+0] = uchar(saturatef(id0[i])*255);
        od[i*4+1] = uchar(saturatef(id1[i])*255);
        od[i*4+2] = uchar(saturatef(id2[i])*255);
        od[i*4+3] = uchar(saturatef(id3[i])*255);
    }
}
...
/*
    recursive Gaussian filter (RGBA uint)
